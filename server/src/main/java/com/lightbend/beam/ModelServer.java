...
import com.lightbend.beam.processors.ConvertDataRecordFunction;
import com.lightbend.beam.processors.ConvertModelRecordFunction;
import com.lightbend.beam.processors.ScoringFunction;
import com.lightbend.beam.processors.SimplePrinterFn;
...
public class ModelServer {
    ...
    public static void main(String[] args) {
        ...
        PCollection<KV<String, DataWithModel>> dataStream = p
                ...
                .apply("Parse data records", ParDo.of(new ConvertDataRecordFunction()));

        // Models Stream - get model records from Kafka
        // It has to use KV to work with state, which is always KV
        PCollection<KV<String,DataWithModel>> modelStream = p
                ...
                .apply("Parse model", ParDo.of(new ConvertModelRecordFunction()));

        // Create a combined PCollection stream combining both model and data streams.
        PCollection<KV<String,DataWithModel>> combinedStream = PCollectionList.of(dataStream).and(modelStream)
                // Flatten the list
        ...
        PCollection<Double> scoringResults = combinedStream
                ...
                .apply("Scoring the model", ParDo.of(new ScoringFunction()))
                // Print scoring result
                .apply("Print result", MapElements.via(new SimplePrinterFn<>()));
